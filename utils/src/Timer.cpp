...
double Timing::GetTotalSeconds(size_t handle) {
    if(handle >= Instance().timers_.size())
    {
        std::cerr << "Error: timers list don't have this timer!!!" << std::endl;
        return 0.;
        // std::out_of_range("timers list don't have this timer!!!");
    }
...
double Timing::GetRecentSeconds(std::string const &tag)
{
    GetRecentSeconds(GetHandle(tag));
}

double Timing::GetRecentSeconds(size_t handle)
{
    if(handle >= Instance().timers_.size())
    {
        std::cerr << "Error: timers list don't have this timer!!!" << std::endl;
    ...
    return Instance().timers_[handle].acc_.Recent();
}
